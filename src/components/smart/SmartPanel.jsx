...
export function SmartPanel({ children, className = "" }) {
  return (
    <section className={`min-w-0 max-w-full rounded-[1.75rem] border border-white/55 bg-white/72 p-4 shadow-soft backdrop-blur-xl dark:border-white/10 dark:bg-white/8 sm:p-5 ${className}`}>
      {children}
    </section>
  );
}
...
export function SmartCard({ icon: Icon, title, description, metric, action, onClick, className = "" }) {
  ...
  return (
    <motion.button
      type="button"
      onClick={onClick}
      className={`group min-w-0 max-w-full w-full rounded-[1.75rem] border border-white/60 bg-white/74 p-5 text-left shadow-soft backdrop-blur-xl transition hover:-translate-y-0.5 hover:border-brass/45 hover:bg-brass/10 dark:border-white/10 dark:bg-white/8 dark:hover:bg-brass/12 ${className}`}
