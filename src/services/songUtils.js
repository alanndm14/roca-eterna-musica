...
export function getEffectiveThemeMode(themeMode = "system") {
...
export function normalizeSong(song = {}, keyPreference = "sharps") {
  ...
  return {
    ...
    originalBpm: Number.isFinite(Number(song.originalBpm || song.tempo)) ? Math.min(240, Math.max(0, Math.round(Number(song.originalBpm || song.tempo)))) : 0,
    timeSignature: ["2/4", "3/4", "4/4", "6/8"].includes(song.timeSignature) ? song.timeSignature : "",
    originalEntryNote: song.originalEntryNote || "",
    churchBpm: Number.isFinite(Number(song.churchBpm)) ? Math.min(240, Math.max(0, Math.round(Number(song.churchBpm)))) : 0,
    churchTimeSignature: ["2/4", "3/4", "4/4", "6/8"].includes(song.churchTimeSignature) ? song.churchTimeSignature : "",
    churchEntryNote: song.churchEntryNote || "",
    capo,
    keyWithCapo,
    ...
    coverBackgroundOpacity: Number.isFinite(Number(song.coverBackgroundOpacity))
      ? Math.min(100, Math.max(0, Math.round(Number(song.coverBackgroundOpacity))))
      : 22,
